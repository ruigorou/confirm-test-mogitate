    ...
    public function run()
    {
        $products = [
            ...
            [
                ...
                'price' => 850,
                'image' => 'orange.png',
                'description' => '当店では酸味と甘みのバランスが抜群のネーブルオレンジを使用しています。酸味は控えめで、甘さと 濃厚な果汁が魅力の商品です。もぎたてフルーツのスムージをお召し上がりください！',
            ],
            [
                'name' => 'スイカ',
                'price' => 700,
        ...
        foreach ($season_list as $season) {
            $product = Product::find($season['product_id']);
            if ($product) {
                $product->seasons()->attach($season['season_id']);
            }
        };
    }
}
