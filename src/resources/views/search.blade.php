    ...
    <div class="content">
        <div class="content_form">
            <form class="search_form" action="/products/search" method="GET">
                @csrf
                <input class="search_category__input" type="text" name="name" placeholder="商品名で検索" value="{{ request('name') }}">
            ...
            <form action="{{ route('search') }}" method="GET">
                ...
                @if(request('sort'))
                    <div class="sort-condition">
                        ...
                        <a href="{{ route('search', request()->except(['sort', 'page']))}}" class="close-btn">×</a>
                    </div>
                @endif
            </form>
        </div>
